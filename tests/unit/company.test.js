...
const BITDEFENDER_ANAF_RECORD = {
  cui: 18189442,
  name: 'BITDEFENDER SRL',
  address: 'BUCUREŞTI, SECTOR 1, STR. FABRICII NR. 44, CLĂDIREA CENTER 1, ET. 1, CAMERA 7',
  caenCode: '5829',
  inactive: false,
  registrationNumber: 'J40/19002/2007',
  vatRegistered: true,
  ...
  authorizedCaenCodes: ['5829', '6201', '6202', '6209', '6311', '6391', '7022']
};
...
describe('company.js', () => {
  ...
  describe('getCompanyBrand', () => {
    it('should return the company brand', () => {
      ...
      expect(brand).toBe('BITDEFENDER');
    });
  });

  describe('getCompanyData (no cache)', () => {
    it('should find BITDEFENDER via ANAF search and return company data', async () => {
      mockFetch
        .mockResolvedValueOnce(anafSearchResponse([
          { cui: 18189442, name: 'BITDEFENDER SRL', statusLabel: 'Funcțiune' }
        ]))
        .mockResolvedValueOnce(anafCompanyResponse(BITDEFENDER_ANAF_RECORD));

      const result = await company.getCompanyData();

      expect(result).toHaveProperty('company', 'BITDEFENDER SRL');
      expect(result).toHaveProperty('cif', '18189442');
      expect(result).toHaveProperty('active', true);
      expect(result).toHaveProperty('anafData');
      expect(result.anafData.name).toBe('BITDEFENDER SRL');
    });
    ...
    it('should pick first active company when no exact match', async () => {
      ...
      expect(result.cif).toBe('18189442');
    ...
    it('should throw when no active company found', async () => {
      mockFetch.mockResolvedValueOnce(anafSearchResponse([
        { cui: 11111111, name: 'BITDEFENDER SOMETHING SRL', statusLabel: 'Radiată' }
      ]));
    ...
    it('should throw when ANAF returns no company name', async () => {
      mockFetch
        ...
        .mockResolvedValueOnce(anafCompanyResponse({ cui: 18189442, name: null }));
  ...
  describe('validateAndGetCompany', () => {
    it('should return company data with status active', async () => {
      mockFetch
        ...
        .mockResolvedValueOnce(peviitorResponse([{ company: 'BITDEFENDER SRL' }]));
    ...
    it('should return inactive status when company is inactive', async () => {
      const inactiveRecord = { ...BITDEFENDER_ANAF_RECORD, inactive: true };
